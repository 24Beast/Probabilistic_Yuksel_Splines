...
"""
Make First Attempt using Parameteric Form - 21/2 (Done!)

Second attempt using Autoregressive (Neural Network) model - 24/2 (Done!)
...
class YukselSpline(nn.Module):
    """
    Only defined for 2 dimensional data currently.
    """
    ...
    def initLayers(self):
        ...
        self.linear6 = nn.Linear(16, 5)  # (mu_x, mu_y, sigma_x, sigma_y, sigma_xy)
        self.relu = nn.ReLU()
    ...
    def transform(self, P, t_val):
        ...
        ----------
        P : torch.tensor
            Tensor containing the control points.
            Shape of the form (Batch size, num_dimensions=2).

        t_val : torch.tensor
            Time data. Assumption t_val ranges from 0-1.
            Shape of the form (Batch_size)
        ...
        """
        mean_vals = self.computeMeanTransform(P[:2], t_val)
        var_vals = self.computeVarTransform(P[2:], t_val)
        return mean_vals, var_vals

    def computeMeanTransform(self, P, t_val):
        P_new = P[:3]
    ...
    def computeVarTransform(self, P, t_val):
        P_new = P[:3]
        # Use transform spline here
        probs = torch.zeros((len(t_val), self.num_dimensions))
        for i in range(self.num_splines - 1):
